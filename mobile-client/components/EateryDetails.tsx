...
export default function EateryDetails({ eatery } : Props) {
  ...
  const { contact: { opening_hours: { weekday_text }, phone_number, website }, location: { address }, tags } = eatery;
  ...
  return (
    <>
      <View style={styles.eateryDetails}>
        ...
        <Text style={styles.details}>{weekday_text[calculateDayIndex(date.getDay() - 1)]}</Text>
        <Pressable style={[styles.details, styles.openingHours]} onPress={() => setShowOpeningHours(!showOpeningHours)}>
        ...
        <Text style={styles.details}>{website.replace(/^(https?:\/\/)?/, '').replace(/\/$/, '')}</Text>
      </View>
      <View style={styles.eateryDetails}>
        ...
        <View style={styles.categories}>
          {tags.map((tag: string, index: number) => {
            return <View key={index} style={styles.categoryWrapper}>
